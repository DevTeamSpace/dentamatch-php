  ...
  <div class="media jobCatbox">
    <div class="media-left ">
      <div class="img-holder ">
        ...
        @if($seeker['job_type']==\App\Enums\JobType::TEMPORARY)
          <span id="fav_{{ $seeker['seeker_id'] }}" onclick="markFavourite({{ $seeker['seeker_id'] }});"
    ...
    <div class="media-body row">
      <div class="col-sm-7 pd-t-10 ">
        <div>
          <a href="{{ url('job/seekerdetails/'.$seeker['seeker_id'].'/'.$job['id']) }}"
             class="algn-rel media-heading">{{ $seeker['first_name'].' '.$seeker['last_name'] }}</a>
        ...
        <form action="{{ url('job/updateStatus') }}" method="post">
          ...
          @if($key==\App\Enums\JobAppliedStatus::HIRED)
            @if($seeker['seeker_block']==0 && $seeker['recruiter_block']==0)
              <button type="button" class="modalClick btn btn-primary pd-l-30 pd-r-30 mr-r-5" data-toggle="modal"
                      data-target="#ShortListMessageBox" data-seekerId="{{ $seeker['seeker_id'] }}">Message
              </button>
            @endif
            @if($seeker['job_type']==\App\Enums\JobType::TEMPORARY && ($seeker['ratingId']!=$seeker['seeker_id']))
              @if(!empty($dates) && date("Y-m-d")>$dates[$seekerDatesCount-1])
                <button type="button" class="btn  btn-primary-outline active pd-l-30 pd-r-30 " data-toggle="modal"
                        data-target="#ratesekeerPopup_{{ $seeker['seeker_id'] }}">Leave a Rating
                </button>
              @else
          ...
          @elseif($key==\App\Enums\JobAppliedStatus::SHORTLISTED)
            ...
            <button type="submit" name="appliedStatus" value="{{ \App\Enums\JobAppliedStatus::HIRED }}"
                    class="btn btn-primary pd-l-30 pd-r-30 ">Hire
            </button>
          @elseif($key==\App\Enums\JobAppliedStatus::APPLIED)
            <button type="submit" name="appliedStatus" value="{{ \App\Enums\JobAppliedStatus::REJECTED }}"
                    class="btn btn-link  mr-r-5">Reject
            </button>
            <button type="submit" name="appliedStatus" value="{{ \App\Enums\JobAppliedStatus::SHORTLISTED }}"
                    class="btn btn-primary pd-l-30 pd-r-30 ">Interviewing
            </button>
