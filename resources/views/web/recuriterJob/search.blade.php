...
@endif
    ...
    <div class="row sec-mob">
      <div class="col-sm-6 mr-b-10 col-xs-6">
        <div class="section-title" id="resultFound">{{$seekersList['paginate']->total()}}
          Result{{ (($seekersList['paginate']->total()>1)?'s':'') }} Found
        </div>
      </div>
      <div class="col-sm-6 text-right mr-b-10 col-xs-6">
        <button type="button" class="btn {{ !empty($searchData['avail_all']) ? "btn-primary" : "btn-primary-outline" }}"
                id="availAllBtn">Available all days
        </button>
      </div>
    </div>
    ...
    @if(count($seekersList['paginate'])>0)
      <div class="jobseeker-statebox" id="ajaxData">
        @include('web.recuriterJob.search-seekers-list')
      </div>
    @else
