        ...
        <div class="media jobCatbox">
          <div class="media-left ">
            <div class="img-holder">
              ...
              <img class="media-object img-circle wd-66" src="{{ url("image/66/66/?src=" .$fav->profile_pic) }}"
                   alt="...">
            </div>
          </div>
          <div class="media-body row">
            <div class="col-sm-8 pd-t-10 ">
                    ...
                    @if(round(($fav->sum),0) > 3)
                       ...
                       @if(!empty($fav->sum))
                         <a href="{{ url('jobseeker/'.$fav->seeker_id) }}"
                            class="media-heading">{{$fav->first_name}} {{$fav->last_name}}</a>
                         <span class=" dropdown-toggle label {{$avgrateClass}}"
                               data-toggle="dropdown">{{number_format($fav->sum, 1, '.', '')}}</span>
                       @else
                         <a href="{{ url('jobseeker/'.$fav->seeker_id) }}"
                            class="media-heading">{{$fav->first_name}} {{$fav->last_name}}</a>
                      ...
                      <li><div class="rating_on"> Punctuality <span class="ex-text">(Did they show up & were they on time?)</span></div>
                        ...
                        <label class="total-count "><span
                                  class="counter">{{round(($fav->punctuality),0)}}</span>/5</label>
                    </li>
            ...
            <div class="col-sm-4 pd-t-15 text-right">
              <button type="button" class="btn btn-primary pd-l-30 pd-r-30 mr-r-20"
                      onclick="markFavourite('{{$fav->seeker_id}}')">Unfavorite
              </button>
              <button type="button" class="btn btn-primary-outline pd-l-30 pd-r-30 "
                      onclick="putValue('{{$fav->seeker_id}}')">Invite
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="modal-body ">
            ...
            <form data-parsley-validate action="/invite-jobseeker" id="invite_seeker" method="post">
              ...
              <div class="form-group custom-select">
                ...
                <select id="selectJobSeeker" name="selectJobSeeker" class="selectpicker" required=""
                        data-parsley-required-message="Please select the job.">
                </select>
              </div>
              <div class="text-right mr-t-20 mr-b-30">
                <button type="button" class="modalClick btn btn-link mr-r-20" data-toggle="modal"
                        data-target="#jobTemplate">Create Job
                </button>
