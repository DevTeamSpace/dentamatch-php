...
<nav class="customNav navbar navbar-default navbar-fixed-top">
  ...
  <div class="container pos-rel">
    ...
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="{{asset('users/dashboard')}}"><img src="{{asset('web/images/dentaMatchLogo.png')}}"
                                                                       alt=""></a>
    </div>
    ...
    <ul class=" topIconBox navbar-right customnavRight">
            ...
            @if(!empty($notificationList['data']))
              @foreach($notificationList['data'] as $notification)
                ...
                <li>
                  ...
                  <a href="{{ url($notification->id.'/delete-notification') }}"><i
                            class="icon icon-deleteicon notificationdelIcon"></i></a>
                </li>
              @endforeach
            @endif
          </ul>
      ...
      <li class="dropdown profile-name-mob">
        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true"
           aria-expanded="false">{{ Session::get('userData.profile.office_name') }} <span
                  class="fa fa-angle-down"></span></a>
        <ul class="dropdown-menu menuLastBox borderNone">

          <span class="fa fa-caret-up notificationCaret"></span>
          <li><span class="icon icon-account-circle navRightIcon"></span><a
                    href="{{ url('edit-profile') }}"><b>{{Auth::user()->email}}</b><br>
              <button type="button" class="btn btn-primary rghtMenuBtn pd-l-10 pd-r-10">View Profile</button>
            </a>

          </li>
          <li><span class="icon icon-drive-document navRightIcon"></span><a
                    href="{{ url('reports/false') }}">Reports</a></li>
          <li><span class="icon icon-lock navRightIcon"><a href="#"></a></span><a href="{{ url('change-password') }}">Change
              Password</a></li>
          <li><span class="icon icon-drive-form navRightIcon"></span><a href="{{ url('setting-subscription') }}">Subscription
              Details</a></li>
          <li><span class="icon icon-text-document-black-interface-symbol navRightIcon"></span><a
                    href="{{ url('setting-terms-conditions') }}">Terms &amp; Conditions</a></li>
          <li><span class="icon icon-text-document-black-interface-symbol navRightIcon"></span><a
                    href="{{ url('setting-privacy-policy') }}">Privacy Policy</a></li>
          <li><span class="icon icon-logout-web-button navRightIcon"></span><a href="{{ url('logout') }}">Logout</a>
          </li>
