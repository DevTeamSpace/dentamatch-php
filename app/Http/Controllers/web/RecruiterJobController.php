...
class RecruiterJobController extends Controller
{
    ...
    public function sendPushUser($jobstatus, $sender, $receiverId, $jobId)
    {
        ...
        } else if (!$deviceModel && $jobstatus == JobAppliedStatus::INVITED) {
            $email = User::where('id', $receiverId)->first();
            $name = JobSeekerProfiles::where('user_id', $receiverId)->first();
    ...
    private function updateJob($jobType, $allData, $office = '')
    {
        try {
            ...
            } else if ($jobType == config('constants.TemporaryJob')) {
