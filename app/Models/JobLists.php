...
class JobLists extends Model {
    ...
    public static function listJobsByStatus($reqData) {
        ...
        $total = $searchQueryObj->distinct('job_lists.recruiter_job_id')->count('job_lists.recruiter_job_id');
        $page = $reqData['page'];
        ...
        $searchResult = $searchQueryObj->groupby('job_lists.recruiter_job_id')->skip($skip)->take($limit)->get();
        $result = array();
        if ($searchResult) {
